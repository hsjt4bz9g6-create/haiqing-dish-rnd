...
**页面说明**：
- **菜品研发页面**（/）：社媒洞察 + AI菜品研发（辅料支持动态添加/删除）
...
```
用户："帮我研发一道香煎鳕鱼，主料鳕鱼200g，辅料黄油20g，烹饪方法是香煎至两面金黄"
```
...
#### 技术实现
- **社媒洞察节点**：`src/graphs/nodes/social_media_insight_node.py`
  - 使用SearchClient搜索网络内容和图片
  - 提取关键词、简介、图片URL
  
- **菜品研发节点**：`src/graphs/nodes/dish_development_node.py`
  - 使用ImageGenerationClient生成菜品图片
  - 使用LLMClient生成菜品卖点
  - 模型：doubao-seed-1-8-251228

#### 工作流定义
- **状态定义**：`src/graphs/state.py`（DishRnDGraphInput/Output）
- **主图编排**：`src/graphs/graph.py`（dish_rnd_graph）
